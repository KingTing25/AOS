PRO rkasc8, Nt, v_scale, r_scale, sp
n=0
tt=dblarr(Nt+1)
Energy=dblarr(Nt+1)
Errorx=dblarr(Nt+1)
...
;x_=x2_+[-r2,0.,0.]
v_=[0., v_scale*((-1)*sqrt(G*M2/r2)) - sqrt(G*Mtot/sqrt(total(x_^2))), 0.] ;prograde/retrograde
...
;while ((total(x1_) gt -3.7218d13) and (it lt Nt)) do begin
  ...
  h=dt
  calcf, x_,v_,P_,tt(it),x_p,v_p
  xk1_n = dt*x_p
  vk1_n = dt*v_p
  
  calcf, x_ + b21*xk1_n, v_ + b21*vk1_n,P_,tt(it)+a2*dt,x_p,v_p
  xk2_n = dt*x_p
  vk2_n = dt*v_p
  
  calcf, x_ + b31*xk1_n + b32*xk2_n, v_ + b31*vk1_n + b32*vk2_n,P_,tt(it)+a3*dt,x_p,v_p
  xk3_n = dt*x_p
  vk3_n = dt*v_p
  
  calcf, x_ + b41*xk1_n + b42*xk2_n + b43*xk3_n, v_ + b41*vk1_n + b42*vk2_n + b43*vk3_n,P_,tt(it)+a4*dt,x_p,v_p
  xk4_n = dt*x_p
  vk4_n = dt*v_p
  
  calcf, x_ + b51*xk1_n + b52*xk2_n + b53*xk3_n + b54*xk4_n, v_ + b51*vk1_n + b52*vk2_n + b53*vk3_n + b54*vk4_n,P_,tt(it)+a5*dt,x_p,v_p
  xk5_n = dt*x_p
  vk5_n = dt*v_p
  
  calcf, x_ + b61*xk1_n + b62*xk2_n + b63*xk3_n + b64*xk4_n + b65*xk5_n, v_ + b61*vk1_n + b62*vk2_n + b63*vk3_n + b64*vk4_n + b65*vk5_n,P_,tt(it)+a6*dt,x_p,v_p
  xk6_n = dt*x_p
  vk6_n = dt*v_p
  ...
  if errmax gt eps then begin ;failed step
    ...
    tt(it+1)=tt(it)+dt
    it=it+1
    if sp eq 1 then printf,1,tt(it),x1_(0),x1_(1),x1_(2),x2_(0),x2_(1),x2_(2),x_(0),x_(1),x_(2)
    ;n=n+1
    calc_binary_pos, P_,tt(it),x1_,x2_
...
close,1
;print,Cj(10),Cj(it)
;print,Cj(it)-Cj(10),(Cj(it)-Cj(10))/Cj(10)
!P.charsize = 1.5
;window,1
;plot,tt/Torb,(Cj-mean(Cj))/mean(Cj),xrange=[tt(10),tt(it)]/Torb
;window,2
;plot,tt/Torb,deriv(tt),xrange=[0.,tt(it)/Torb]
;window,3
;plot,tt/Torb,r2_,xrange=[0,tt(it)/Torb]
print,v_scale,max(r2_)/a, (max(r2_) - min(r2_))/(max(r2_) + min(r2_)), (max(r2_) - min(r2_))/a
if (sp eq 1) then STOP
END

PRO calcf, x_,v_,P_,tt,x_p,v_p
G=6.67d-8
calc_binary_pos,P_,tt,x1_,x2_
Mtot=P_(6)+P_(7)
mu1 = P_(6)/Mtot
mu2 = P_(7)/Mtot
x_p=v_
r23=total(((x_-x2_)*(x_-x2_)))^0.5
r13=total(((x_-x1_)*(x_-x1_)))^0.5
v_p=G*Mtot*mu1/r13^3*(x1_-x_) + G*Mtot*mu2/r23^3*(x2_-x_)
END