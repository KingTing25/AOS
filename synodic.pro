...
dx_=x3_-x1_
dv_=v3_-v1_
hh=total([dx_(1)*dv_(2)-dx_(2)*dv_(1),dx_(2)*dv_(0)-dx_(0)*dv_(2),dx_(0)*dv_(1)-dx_(1)*dv_(0)]^2)
e=(1-hh/(G*(M1+M2)*a))^0.5
r12=a*(1-e*cos(ang))
tt(0)=0
...
;while ((total(x1_) gt -3.7218d13) and (it lt Nt)) do begin
  it=it+1
  h=dt
  calcf, e,ang(it-1),x1_,v1_,x2_,v2_,x3_,v3_,x1_p,v1_p,x2_p,v2_p,x3_p,v3_p,M1,M2,M3
  x1k1_n = dt*x1_p
  x2k1_n = dt*x2_p
  x3k1_n = dt*x3_p
  v1k1_n = dt*v1_p
  ...
  calcf, e, ang(it-1), x1_ + b21*x1k1_n, v1_ + b21*v1k1_n, x2_ + b21*x2k1_n, v2_ + b21*v2k1_n, x3_ + b21*x3k1_n, v3_ + b21*v3k1_n, x1_p,v1_p,x2_p,v2_p,x3_p,v3_p,M1,M2,M3
  x1k2_n = dt*x1_p
  x2k2_n = dt*x2_p
  x3k2_n = dt*x3_p
  v1k2_n = dt*v1_p
  ...
  calcf, e, ang(it-1), x1_ + b31*x1k1_n + b32*x1k2_n, v1_+ b31*v1k1_n + b32*v1k2_n, x2_ + b31*x2k1_n + b32*x2k2_n, v2_ + b31*v2k1_n + b32*v2k2_n, x3_ + b31*x3k1_n + b32*x3k2_n, v3_ + b31*v3k1_n + b32*v3k2_n, x1_p,v1_p,x2_p,v2_p,x3_p,v3_p,M1,M2,M3
  x1k3_n = dt*x1_p
  x2k3_n = dt*x2_p
  x3k3_n = dt*x3_p
  v1k3_n = dt*v1_p
  ...
  calcf, e, ang(it-1), x1_ + b41*x1k1_n + b42*x1k2_n + b43*x1k3_n, v1_+ b41*v1k1_n + b42*v1k2_n + b43*v1k3_n, x2_ + b41*x2k1_n + b42*x2k2_n + b43*x2k3_n, v2_ + b41*v2k1_n + b42*v2k2_n + b43*v2k3_n, x3_ + b41*x3k1_n + b42*x3k2_n + b43*x3k3_n, v3_ + b41*v3k1_n + b42*v3k2_n + b43*v3k3_n, x1_p,v1_p,x2_p,v2_p,x3_p,v3_p,M1,M2,M3
  x1k4_n = dt*x1_p
  x2k4_n = dt*x2_p
  x3k4_n = dt*x3_p
  v1k4_n = dt*v1_p
  ...
  calcf, e, ang(it-1), x1_ + b61*x1k1_n + b62*x1k2_n + b63*x1k3_n + b64*x1k4_n + b65*x1k5_n, v1_+ b61*v1k1_n + b62*v1k2_n + b63*v1k3_n + b64*v1k4_n + b65*v1k5_n, x2_ + b61*x2k1_n + b62*x2k2_n + b63*x2k3_n + b64*x2k4_n + b65*x2k5_n, v2_ + b61*v2k1_n + b62*v2k2_n + b63*v2k3_n + b64*v2k4_n + b65*v2k5_n, x3_ + b61*x3k1_n + b62*x3k2_n + b63*x3k3_n + b64*x3k4_n + b65*x3k5_n, v3_ + b61*v3k1_n + b62*v3k2_n + b63*v3k3_n + b64*v3k4_n + b65*v3k5_n, x1_p,v1_p,x2_p,v2_p,x3_p,v3_p,M1,M2,M3
  x1k6_n = dt*x1_p
  x2k6_n = dt*x2_p
  x3k6_n = dt*x3_p
  v1k6_n = dt*v1_p
  ...
  if errmax gt eps then begin ;failed step
    ...
    ;endelse
    dt=h
    ang(it)=ang(it-1)+total(abs(x1_-x1_n))*2/(2*a)
    x1_ = [a*(1-e*cos(ang)),0d,0d]
    x2_ = [-a*(1-e*cos(ang)),0d,0d]
    x3_ = x3_n
    v1_ = [0d,0d,0d]
    v2_ = [0d,0d,0d]
    v3_ = v3_n
    ;x1_ = [x1_n(0)*cos(ang)-x1_n(1)*sin(ang),x1_n(0)*sin(ang)+x1_n(1)*cos(ang),x1_n(2)]
    ;x2_ = [x2_n(0)*cos(ang)-x2_n(1)*sin(ang),x2_n(0)*sin(ang)+x2_n(1)*cos(ang),x2_n(2)]
    ;x3_ = [x3_n(0)*cos(ang)-x3_n(1)*sin(ang),x3_n(0)*sin(ang)+x3_n(1)*cos(ang),x3_n(2)]
    ;v1_ = [v1_n(0)*cos(ang)-v1_n(1)*sin(ang),v1_n(0)*sin(ang)+v1_n(1)*cos(ang),v1_n(2)]
    ;v2_ = [v2_n(0)*cos(ang)-v2_n(1)*sin(ang),v2_n(0)*sin(ang)+v2_n(1)*cos(ang),v2_n(2)]
    ;v3_ = [v3_n(0)*cos(ang)-v3_n(1)*sin(ang),v3_n(0)*sin(ang)+v3_n(1)*cos(ang),v3_n(2)]
...
PRO calcf, e,ang,x1_k,v1_k,x2_k,v2_k,x3_k,v3_k,x1_p,v1_p,x2_p,v2_p,x3_p,v3_p,M1,M2,M3
G=6.67d-8
r12=7.5d13*(1-e*cos(ang))
...
v3_p=(G*M1/r13^3*(x1_k-x3_k) + G*M2/r23^3*(x2_k-x3_k))*7.5d13/r12
END